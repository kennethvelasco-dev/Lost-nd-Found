...
def get_published_found_items(
    limit=20, offset=0, categories=None
...
def verify_report_db(report_id, entity_type, decision, reason, admin_username):
    ...
    query = text(
        f"UPDATE {table} SET status = :status, rejection_reason = :reason WHERE id = :id"
    )  # nosec B608
    res = db.session.execute(
        query, {"status": new_status, "reason": reason, "id": report_id}
    )
    if res.rowcount == 0:
...
def get_user_reports_db(user_id):
    l_query = text(
        "SELECT *, last_seen_datetime AS incident_date, created_at AS report_date, 'lost' as type FROM lost_items WHERE reporter_id = :uid AND (is_dismissed = FALSE OR is_dismissed IS NULL)"
    )
    f_query = text(
        "SELECT *, found_datetime AS incident_date, created_at AS report_date, 'found' as type FROM found_items WHERE reporter_id = :uid AND (is_dismissed = FALSE OR is_dismissed IS NULL)"
    )

    lost = [
        dict(row._mapping)
        for row in db.session.execute(l_query, {"uid": user_id}).fetchall()
    ]
    found = [
        dict(row._mapping)
        for row in db.session.execute(f_query, {"uid": user_id}).fetchall()
    ]
    return lost + found
...
def dismiss_report_db(report_id, entity_type, user_id):
    table = "lost_items" if entity_type == "lost" else "found_items"
    query = text(
        f"UPDATE {table} SET is_dismissed = TRUE WHERE id = :id AND reporter_id = :uid"
    )  # nosec B608
...
def search_items_db(filters: Dict[str, Any]) -> tuple[list[Dict[str, Any]], int]:
    ...
    if filters.get("query"):
        c_common += (
            " AND (public_description LIKE :q OR category LIKE :q OR item_type LIKE :q)"
        )
    ...
    if status in ["returned", "lost"]:
        ...
        count_query = text(f"SELECT COUNT(*) FROM {table} {where_clause}")  # nosec B608
        total_count = db.session.execute(count_query, params).scalar() or 0

        select_query = text(
            f"SELECT * FROM {table} {where_clause} ORDER BY created_at DESC LIMIT :limit OFFSET :offset"
        )  # nosec B608
        result = db.session.execute(select_query, params).fetchall()
...
def get_released_items_db(limit=20, offset=0, query=None):
    ...
    if query:
        where_clause = (
            "WHERE claimant_name LIKE :q OR category LIKE :q OR item_type LIKE :q"
        )
        params["q"] = f"%{query}%"
...
def get_pending_reports_db():
    l_q = text(
        "SELECT *, 'lost' as type FROM lost_items WHERE status = 'pending_approval'"
    )
    f_q = text(
        "SELECT *, 'found' as type FROM found_items WHERE status = 'pending_approval'"
    )
...
def get_dashboard_stats_db():
    total_lost = (
        db.session.execute(
            text("SELECT COUNT(*) FROM lost_items WHERE status = 'reported_lost'")
        ).scalar()
        or 0
    )
    total_found = (
        db.session.execute(
            text("SELECT COUNT(*) FROM found_items WHERE status = 'found'")
        ).scalar()
        or 0
    )
    pending_claims = (
        db.session.execute(
            text("SELECT COUNT(*) FROM claims WHERE decision = 'pending'")
        ).scalar()
        or 0
    )
    resolved = (
        db.session.execute(text("SELECT COUNT(*) FROM released_items")).scalar() or 0
    )
