...
def create_claim(data):
    ...
    try:
        ...
        existing = None
        if found_item_id:
            query = text("SELECT id FROM claims WHERE user_id = :user_id AND found_item_id = :item_id AND decision = 'pending'")
            existing = db.session.execute(query, {"user_id": data.get("user_id"), "item_id": found_item_id}).fetchone()
        elif lost_item_id:
            query = text("SELECT id FROM claims WHERE user_id = :user_id AND lost_item_id = :item_id AND decision = 'pending'")
            existing = db.session.execute(query, {"user_id": data.get("user_id"), "item_id": lost_item_id}).fetchone()
        ...
        import traceback
        logger.error(f"CRITICAL ERROR in create_claim: {str(e)}\n{traceback.format_exc()}")
...
def link_claim_to_found_item(claim_id, found_item_id):
    ...
    try:
        ...
        update_query = text("""
            ...
            WHERE id = :id
        """)
        db.session.execute(update_query, {"f_id": found_item_id, "score": score, "id": claim_id})
        db.session.commit()
    ...
    try:
        ...
        for key, value in data.items():
            if key in ALLOWED_FIELDS:
                ...
                params[key] = json.dumps(value) if key == "answers" and not isinstance(value, str) else value

        if not updates:
            return {"error": "No valid fields to update"}, 400

        query = text(f"UPDATE claims SET {', '.join(updates)} WHERE id = :id")
...
def verify_claim(claim_id, decision, admin_username, handover_notes=None):
    try:
        ...
        select_query = text("SELECT found_item_id, lost_item_id, decision FROM claims WHERE id = :id")
        ...
        if decision == "approved":
            ...
            if item_id:
                ...
                check_query = text(f"SELECT id FROM claims WHERE {col} = :item_id AND decision IN ('approved', 'completed') AND id != :id")
                other = db.session.execute(check_query, {"item_id": item_id, "id": claim_id}).fetchone()
                if other:
        ...
        if decision == "completed":
            ...
            # Update item status
            if found_item_id:
                db.session.execute(text("UPDATE found_items SET status = 'found' WHERE id = :id"), {"id": found_item_id})
            elif lost_item_id:
                db.session.execute(text("UPDATE lost_items SET status = 'found' WHERE id = :id"), {"id": lost_item_id})
        else:
            if current_decision not in ("pending", "approved"):
                return {"error": f"Claim already processed (Current status: {current_decision})"}, 400
            db.session.execute(text("UPDATE claims SET decision = :decision WHERE id = :id"), {"decision": decision, "id": claim_id})
...
def get_claim_detail_db(claim_id):
    ...
    d = dict(row._mapping)
    if d.get("answers"):
        try: d["answers"] = json.loads(d["answers"]) if isinstance(d["answers"], str) else d["answers"]
        except: pass
    return d
...
def dismiss_claim_db(claim_id, user_id):
    try:
        db.session.execute(text("UPDATE claims SET is_dismissed = TRUE WHERE id = :id AND user_id = :uid"), {"id": claim_id, "uid": user_id})
        db.session.commit()
