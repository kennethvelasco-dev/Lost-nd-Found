...
def submit_claim(data: dict, user_id: str) -> tuple:
    ...
    receipt_proof = data.get("receipt_proof") or data.get(
        "proof"
    )  # Support 'proof' alias
...
def get_potential_matches_service(claim_id: int, user_id: str, role: str) -> tuple:
    ...
    related_cats = (
        get_related_categories(claimed_category) if claimed_category else None
    )
...
def link_claim_service(
    claim_id: int, found_item_id: int, user_id: str, role: str
) -> tuple:
...
def schedule_pickup_service(
    claim_id: int, data: dict, user_id: str, role: str
) -> tuple:
