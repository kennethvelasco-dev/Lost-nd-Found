...
def login_user(data: dict) -> tuple:
    ...
    if user.get("lockout_until"):
        ...
        if datetime.now(timezone.utc) < lockout_time:
            raise ValidationError("Account is locked due to multiple failed attempts. Try again later.", 403)
...
def request_password_reset(email: str):
    ...
    if not user:
        ...
        return {"message": "If an account exists with this email, a reset link has been sent."}, 200
