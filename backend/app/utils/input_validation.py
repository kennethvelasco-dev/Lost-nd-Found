...
def validate_password_strength(password: str) -> bool:
    ...
    if not any(c.isupper() for c in password):
        raise ValidationError(
            "Password must contain at least one uppercase letter", 400
        )
    if not any(c.islower() for c in password):
        raise ValidationError(
            "Password must contain at least one lowercase letter", 400
        )
...
def validate_item_payload(data: dict, mode: str) -> bool:
    ...
    if mode == "lost":
        require_fields(
            data,
            common_required
            + ["last_seen_location", "last_seen_datetime", "private_details"],
        )
    ...
    try:
        ...
        raise ValidationError(
            f"Invalid datetime format for {date_field}. Use ISO format.", 400
        )
