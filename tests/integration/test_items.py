...
def test_post_found_item_user(client, app):
    # Setup: Register user
    user_data = {
        "username": "itemuser", "password": "Password123!",
        "name": "Item User", "email": "item@test.com", "role": "user"
    }
...
def test_search_and_pagination(client, app):
    ...
    admin_data = {
        ...
        "name": "Admin", "email": "admin@test.com", "role": "admin", "admin_id": "ADM-001"
    }
    ...
    for i in range(5):
        item = {
            "category": "Books", "item_type": f"Book {i}",
            "found_location": "Library", "found_datetime": datetime.now(timezone.utc).isoformat(),
