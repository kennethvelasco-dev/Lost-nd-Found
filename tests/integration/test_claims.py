...
def test_full_claim_lifecycle(client, app):
    ...
    admin_data = {
        "username": "admin8", "password": "Password123!",
        "name": "Admin 8", "email": "admin8@test.com", "role": "admin", "admin_id": "ADM-008"
    }
    user_data = {
        "username": "user8", "password": "Password123!",
        "name": "User 8", "email": "user8@test.com", "role": "user"
    }
    resp1 = client.post("/api/auth/register", json=admin_data)
    assert resp1.status_code == 201, f"Admin registration failed: {resp1.get_data(as_text=True)}"
    resp2 = client.post("/api/auth/register", json=user_data)
    assert resp2.status_code == 201, f"User registration failed: {resp2.get_data(as_text=True)}"
    ...
    found_item = {
        "category": "Electronics", "item_type": "Phone", "brand": "Apple", "color": "Silver",
        "found_location": "Cafeteria", "found_datetime": datetime.now(timezone.utc).isoformat(),
    ...
    claim_payload = {
        "category": "Electronics", "item_type": "Phone", "brand": "Apple", "color": "Silver",
        "location": "Cafeteria", "datetime": datetime.now(timezone.utc).isoformat(),
    ...
    resp = client.post(f"/api/claims/{claim_id}/schedule", json=schedule_payload, headers=admin_headers)
    assert resp.status_code == 200
