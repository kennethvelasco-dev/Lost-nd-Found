...
def test_idor_prevention(client, app):
    ...
    u1 = {"username": "s_user1", "password": "Password123!", "name": "U1", "email": "s1@test.com", "role": "user"}
    u2 = {"username": "s_user2", "password": "Password123!", "name": "U2", "email": "s2@test.com", "role": "user"}
    ...
    # User 1 reports
    item = {"category": "Tools", "item_type": "Hammer", "found_location": "Yard", "found_datetime": "2026-03-10T10:00:00", "public_description": "Hammer"}
...
def test_concurrency_double_approval(client, app):
    ...
    admin = {"username": "s_admin", "password": "Password123!", "name": "Admin", "email": "sa@test.com", "role": "admin", "admin_id": "ADM-SEC"}
    ...
    # Create item
    item = {"category": "Tools", "item_type": "Drill", "found_location": "Yard", "found_datetime": "2026-03-10T10:00:00", "public_description": "Drill"}
    ...
    claim_ids = []
    for i in range(3, 5):
        u = {"username": f"s_user{i}", "password": "Password123!", "name": f"U{i}", "email": f"s{i}@test.com", "role": "user"}
        resp_reg = client.post("/api/auth/register", json=u)
        assert resp_reg.status_code == 201
        h = get_auth_headers(client, f"s_user{i}", "Password123!")
        claim = {"found_item_id": item_id, "claimant_name": f"U{i}", "claimant_email": f"s{i}@test.com", "description": f"Mine {i}", "declared_value": "10", "receipt_proof": "N/A"}
    ...
    resp_app1 = client.post(f"/api/claims/{c1}/verify", json={"decision": "approved"}, headers=ha)
    assert resp_app1.status_code == 200
    
    # Try to approve second
    resp_app2 = client.post(f"/api/claims/{c2}/verify", json={"decision": "approved"}, headers=ha)
